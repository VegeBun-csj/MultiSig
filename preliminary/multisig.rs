...
impl<T: Config> Pallet<T> {
	...
	fn get_call(
		...
		maybe_known.map_or_else(
			// 如果没有提供call调用，即提供的是hash值，通过hash值获取到存储Calls的值 (OpaqueCall<T>,T::AccountId, BalanceOf<T>)
			|| {
				Calls::<T>::get(hash)
					// 取得其中的OpaqueCall<T>数据data，并对其进行解码得到Call，和其长度
					.and_then(|(data, ..)| Some((data.try_decode()?, data.encoded_len())))
			},
			// 如果提供了call调用，就直接进行解码，得到call和其长度
